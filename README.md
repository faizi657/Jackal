...
## 🖥️ How to Install Jackal on Windows  
...
2. You might see a security warning from Windows asking if you trust the source. Confirm that you want to run the program.  
3. Jackal will run directly. You do not need to install anything else.  

### If you downloaded a `.zip` folder:  
1. Right-click the `.zip` file.  
2. Select “Extract All...” from the menu.  
3. Choose a folder where you want to unpack the files (for example, a new folder on your Desktop).  
4. Open the extracted folder.  
5. Find the `.exe` file inside and double-click it to start Jackal.  
...
### To start Jackal:  
1. Use the `cd` command to go to the folder where you saved or extracted Jackal. For example:  
   ```  
   cd Desktop\Jackal  
   ```  
2. Type the name of the executable file and press `Enter`. For example:  
   ```  
   jackal.exe  
   ```  
3. Jackal will start running. You can now enter commands to play chess or test moves.  

### Basic commands (example):  
- To start a new game: type `new` and press `Enter`.  
- To make a move: type the chess move in standard notation (e.g., `e2e4`).  
- To exit Jackal: type `quit` and press `Enter`.  

## 🕹️ How to Use Jackal With a Chess GUI (Optional)  
...
1. Download and install a chess GUI on your computer.  
2. Open the GUI.  
3. Look for the option to add or configure an engine.  
4. Select “Add Engine” and browse to the location of the Jackal `.exe` file.  
5. Save the engine settings.  
6. Start a new game in the GUI and select Jackal as your opponent or analysis engine.  

## 🔧 Adjusting Jackal Settings  
